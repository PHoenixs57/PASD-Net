...
RNNModel *pasdnet_model_from_buffer(const void *ptr, int len) {
  RNNModel *model;
...
RNNModel *pasdnet_model_from_filename(const char *filename) {
  RNNModel *model;
  FILE *f = fopen(filename, "rb");
  model = pasdnet_model_from_file(f);
  model->file = f;
  return model;
}

RNNModel *pasdnet_model_from_file(FILE *f) {
  RNNModel *model;
  model = malloc(sizeof(*model));
  model->file = NULL;
...
void pasdnet_model_free(RNNModel *model) {
  if (model->file != NULL) fclose(model->file);
  if (model->blob != NULL) free(model->blob);
  free(model);
}

int pasdnet_get_size(void) {
  return sizeof(DenoiseState);
}

int pasdnet_get_frame_size(void) {
  return FRAME_SIZE;
}

int pasdnet_init(DenoiseState *st, RNNModel *model) {
  ...
  if (model != NULL) {
    ...
    if (list != NULL) {
      ret = init_pasdnet(&st->model, list);
      opus_free(list);
    }
    if (ret != 0) return -1;
  }
    ...
    int ret = init_pasdnet(&st->model, pasdnet_arrays);
    if (ret != 0) return -1;
  }
#endif
...
DenoiseState *pasdnet_create(RNNModel *model) {
  int ret;
  DenoiseState *st;
  st = malloc(pasdnet_get_size());
  ret = pasdnet_init(st, model);
  if (ret != 0) {
    free(st);
    return NULL;
  }
...
void pasdnet_destroy(DenoiseState *st) {
  free(st);
}
...
int rnn_compute_frame_features(DenoiseState *st, kiss_fft_cpx *X, kiss_fft_cpx *P,
  ...
  rnn_pitch_search(pitch_buf+(PITCH_MAX_PERIOD>>1), pitch_buf, PITCH_FRAME_SIZE,
               PITCH_MAX_PERIOD-3*minperiod, &pitch_index);
  pitch_index = PITCH_MAX_PERIOD-pitch_index;

  gain = rnn_remove_doubling(pitch_buf, PITCH_MAX_PERIOD, minperiod,
  ...
  if (!TRAINING && E < silence_E_threshold) {
...
float pasdnet_process_frame(DenoiseState *st, float *out, const float *in) {
  int i;
  ...
  if (!silence) {
    ...
    for (i=0;i<NB_BANDS;i++) {
      ...
      st->lastg[i] = MIN16(1.f, g[i]*(Ex[i]+1e-3)/(Ex[i]+1e-3));
    }
  ...
  /* Still keep updating delayed_* for state recording, in case other parts depend on it. */
