...
unquantized = [ 'conv1', 'dense_out', 'vad_dense', 'attn_dense' ]

description=f"""
This is an unsafe dumping script for PASD-Net models. It assumes that all weights are included in Linear, Conv1d or GRU layer
...
parser.add_argument('--export-filename', type=str, default='pasdnet_data', help='filename for source and header file (.c and .h will be added), defaults to pasdnet_data')
parser.add_argument('--struct-name', type=str, default='PASDNet', help='name for C struct, defaults to PASDNet')
...
if __name__ == "__main__":
    ...
    saved_gen= torch.load(args.weightfile, map_location='cpu', weights_only=False)
    saved_gen['model_args']    = ()
    #saved_gen['model_kwargs']  = {'cond_size': 256, 'gamma': 0.9}

    model = pasd_net.PASDNet(*saved_gen['model_args'], **saved_gen['model_kwargs'])
